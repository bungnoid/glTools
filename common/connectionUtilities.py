...
class ConnectionUtilities(object):
	...
	def connectionListToAttr(self,toNode,toAttr):
		'''
		...
		if not mc.objExists(toNode+'.'+toAttr):
			raise UserInputError('Attribute ' + toNode+'.'+toAttr + ' does not exist!')
			return
	...
	def connectionListFromAttr(self,fromNode,fromAttr):
		'''
		...
		if not mc.objExists(fromNode+'.'+fromAttr):
			raise UserInputError('Attribute ' + fromNode+'.'+fromAttr + ' does not exist!')
			return
	...
	def combineSingleAttrConnections(self,targetAttr,inputAttr1='',inputAttr2='',inputAttr1Value=None,inputAttr2Value=None,combineMode='add',enableAttributeOverride=False,blendAttr=''):
		'''
		...
		if not mc.objExists(targetAttr): raise UserInputError('Target attribute '+targetAttr+' does not exist!')
		# Check inputs
		if (not inputAttr1) and (inputAttr1Value==None): raise UserInputError('No input attribute or value specified for input1!')
		if (not inputAttr2) and (inputAttr2Value==None): raise UserInputError('No input attribute or value specified for input2!')
		# Check input attributes
		if inputAttr1 and not mc.objExists(inputAttr1): raise UserInputError('Input attribute 1 '+inputAttr1+' does not exist!')
		if inputAttr2 and not mc.objExists(inputAttr2): raise UserInputError('Input attribute 2 '+inputAttr2+' does not exist!')
		
		# Get target node name
		if not targetAttr.count('.'): raise UserInputError(targetAttr+' is not a valid attribute!')
		...
		if combineMode == 'blend' and blendAttr:
			...
			if not mc.objExists(blendAttr):
				if not blendAttr.count('.'): raise UserInputError(targetAttr+' is not a valid object.attribute name!')
				blendAttrName = blendAttr.split('.')
				if len(blendAttrName) != 2: raise UserInputError(targetAttr+' is not a valid object.attribute name!')
				if not mc.objExists(blendAttrName[0]): raise UserInputError('Object '+blendAttrName[0]+' does not exist!')
	...
	def replace(self,original,new,inputs=True,outputs=True):
		'''
		...
		'''
		# Check original
		if not mc.objExists(original): raise UserInputError('Object "'+original+'" does not exist!!')
		origAttr = bool(original.count('.'))
		# Check new
		if not mc.objExists(new): raise UserInputError('Object "'+new+'" does not exist!!')
		newAttr = bool(new.count('.'))
		
		# Check argument types
		if origAttr != newAttr: raise UserInputError('Specify 2 nodes or node attributes! Cannot mix nodes and attributes!!')
		asAttr = origAttr
			...
			if asAttr:
				...
				if inputList:
					for i in range(len(inputList)/2):
						...
						if not mc.objExists(newDest): raise UserInputError('Object attribute "'+newDest+'" does not exist!')
			...
			if asAttr:
				...
				if outputList:
					for i in range(len(outputList)/2):
						...
						if not mc.objExists(newOutput): raise UserInputError('Object attribute "'+newOutput+'" does not exist!')
